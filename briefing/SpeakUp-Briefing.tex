...
\midrule
Fragmented workflows & Disconnected mobile, desktop, execution \\
Limited AI in trusted boundaries & Workflow degradation to stay compliant \\
Broadcast email as work proxy & Reduced signal-to-noise; interrupts focus \\
Untracked coordination & Limited traceability and auditability \\
Knowledge attrition risk & Personnel transition loses institutional knowledge \\
\bottomrule
\end{tabular}
\end{table}
\end{frame}
...
\midrule
NASA TReK & Export control burden, requires sponsor & Future path (document constraint) \\
\midrule
NASA JPL ION-DTN & Open source, no restrictions & Selected as default \\
\midrule
Clean-room Python & Full control, no dependencies & Selected for learning/customization \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Method} & \textbf{Application} & \textbf{NIST Control} \\
\midrule
Manual Inspection & Document review for completeness & --- \\
\midrule
PII Pattern Scan & Phone, SSN, IP address detection & SI-12 \\
\midrule
Malware Scan & ClamAV virus/malware detection & SI-3 \\
\midrule
Secrets Scan & API keys, credentials, passwords & SA-11 \\
\midrule
MAC Address Scan & IEEE 802.3 hardware identifiers & SC-8 \\
\midrule
Host Security & OS security configuration & CM-6 \\
\midrule
File Integrity & SHA-256 cryptographic hashes & SI-7 \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{30 commits in 8 person-hours} --- demonstrating AI-assisted productivity:
...
\midrule
\texttt{9e5aca8} & Update documentation and security scan output handling \\
\texttt{f1f386e} & Add master build script \\
\texttt{8855611} & Add long-duration DTN transfer test with LOS/AOS simulation \\
\texttt{4090a0a} & Add BPv7 integration tests and Wireshark capture demo \\
\texttt{f5d1959} & Update TCPCL to RFC 9174 wire format for Wireshark compatibility \\
\texttt{1f3cb42} & Add constraints slide and update documentation \\
\texttt{cd86850} & Add decision log and export control documentation \\
\texttt{6015094} & Add Docker-based DTN simulation environment \\
\texttt{0aecf3c} & Add TCP Convergence Layer v4 (RFC 9174) \\
\texttt{318800a} & Add BPv7 core implementation (CCSDS 734.20-O-1 / RFC 9171) \\
\texttt{acc6be5} & Add automated security verification suite \\
\texttt{2b27e68} & Strengthen value proposition with concrete example \\
\texttt{35a13c7} & Add lifecycle management to Phase 3 workflow \\
\texttt{79ac56e} & Add LaTeX briefing deck with PDF output \\
